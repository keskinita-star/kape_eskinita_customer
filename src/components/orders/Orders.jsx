...
import { listenToCustomerOrders } from "../../services/customerOrderService";
...
export default function Orders() {
  ...
  useEffect(() => {
    const unsub = listenToCustomerOrders(customer.uid, (fetchedOrders) => {
      ...
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e8e2d9", overflow: "hidden" }}>
        ...
        <div style={{ padding: "12px 14px", borderBottom: "1px solid #f5f3ee", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: "#1a1814", marginTop: 2 }}>
              {order.items?.length} item{order.items?.length > 1 ? "s" : ""}
              {order.pickupTime ? ` · Pickup ${order.pickupTime}` : ""}
            </div>
          </div>
          <span style={{ fontSize: 11, fontWeight: 600, padding: "4px 10px", borderRadius: 20, background: st.bg, color: st.color }}>
            {st.label}
          </span>
        </div>
        ...
        <div style={{ padding: "10px 14px" }}>
          {order.items?.slice(0, 2).map((item, i) => (
            <div key={i} style={{ fontSize: 12, color: "#6b6860", marginBottom: 2 }}>
              {item.name} × {item.qty}
              {item.addons?.length > 0 && (
                <span style={{ color: "#9a9690" }}> + {item.addons.map(a => a.label).join(", ")}</span>
              )}
            </div>
          ))}
          {order.items?.length > 2 && (
            <div style={{ fontSize: 12, color: "#9a9690" }}>+{order.items.length - 2} more</div>
          )}
        </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {showTrack && (
              <button onClick={() => navigate(`/orders/${order.id}`)}
                style={{ padding: "6px 14px", borderRadius: 8, background: "#1a1814", color: "#fff", border: "none", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                Track →
              </button>
            )}
            {order.status === "completed" && (
              <button onClick={() => handleReorder(order)}
                style={{ padding: "6px 14px", borderRadius: 8, background: "#f5f3ee", color: "#1a1814", border: "1px solid #e8e2d9", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                Reorder
              </button>
            )}
          </div>
  ...
  return (
    <div style={{ background: "#f5f3ee", minHeight: "100vh", paddingBottom: 80 }}>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 40, color: "#9a9690", fontSize: 13 }}>
          Loading orders...
        </div>
      ) : orders.length === 0 ? (
        ...
        <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {activeOrders.length > 0 && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#9a9690", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.5px" }}>
